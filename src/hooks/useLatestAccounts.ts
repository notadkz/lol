...
import { fetchLatestAccounts } from "@/lib/api";

export function useLatestAccounts(limit: number = 4) {
  const [accounts, setAccounts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const getLatestAccounts = async () => {
      try {
        setIsLoading(true);
        ...
        if (response.data) {
          if (
            ...
            console.log("Keys của response.data:", Object.keys(response.data));
          }
        }
        ...
        if (
          ...
          console.log("Tìm thấy mảng products trong response.data, sử dụng nó");
        ...
        console.warn(
          "Cấu trúc dữ liệu API không theo chuẩn, đang thử phục hồi..."
        );
        ...
        if (
          ...
          console.log(
            "Tìm thấy mảng items trong response.data, sử dụng nó thay thế"
          );
        ...
        if (response.data && typeof response.data === "object") {
          ...
          for (const key of keys) {
            if (Array.isArray(response.data[key])) {
              console.log(
                `Tìm thấy mảng trong response.data.${key}, sử dụng nó thay thế`
              );
        ...
        console.error(
          "Không thể phục hồi dữ liệu, không tìm thấy mảng nào trong response"
        );
        setAccounts([]);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Đã xảy ra lỗi");
        console.error("Error fetching latest accounts:", err);
  ...
  function formatAndSetAccounts(accountsData: any[]) {
    ...
    const formattedAccounts: Product[] = accountsData.map((account: any) => {
      ...
      return {
        ...
        featured: account.featuredUntil
